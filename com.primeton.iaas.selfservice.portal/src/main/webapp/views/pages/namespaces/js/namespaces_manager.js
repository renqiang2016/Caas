	var namespacesGrid = new Datatable();
	var userId = document.getElementById('userId').value ;
	function initNamespacesGrid () {
		namespacesGrid.init({
            src: $("#pageNamespacesGrid"),
            ...
            dataTable: {
                ...
                "ajax": {
                	 url:contextPath+"/rest/caas/namespaces/listUserNamespaces/"+userId,
                	 type:"GET"
                 }, 
                 "columns":[
                     {"data":"check"},
	...
	function addRow(){
		$("#namespacesModal .modal-title").html("新建");
		$("#namespacesModal #saveBtn").css("display","");
		$("#namespacesModal #updateBtn").css("display","none");
    	$('#namespacesModal').modal('show');
	}
	...
	function doSave(type){
    	...
    	$.ajax({
    		...
    		success:function(ret){
    			$('#namespacesModal').modal('hide');
    			namespacesGrid.reload();
    		},
    		error:function(ret){
	...
	function editRow(row){
		$("#namespacesModal .modal-title").html("编辑");
		$("#namespacesModal #saveBtn").css("display","none");
		$("#namespacesModal #updateBtn").css("display","");
		var data = namespacesGrid.getRowData(row.id);		
    ...
    $(function(){
    	Metronic.init(); 
    	initNamespacesGrid();
        $('#namespacesModal').on("hidden.bs.modal", function() {
        	$("#namespacesForm")[0].reset();
        });
        formHelper = new FormHelper("namespacesForm");
    });
