	...
	function addRow(){
		$("#funcModal .modal-title").html("新建");
		$("#funcModal #saveBtn").css("display","");
		$("#funcModal #updateBtn").css("display","none");
    	$('#funcModal').modal('show');
	}
	
	function editRow(row){
		$("#funcModal .modal-title").html("编辑");
		$("#funcModal #saveBtn").css("display","none");
		$("#funcModal #updateBtn").css("display","");
    ...
    $(function(){
    	Metronic.init(); 
    	inituserGrid();
        $('#funcModal').on("hidden.bs.modal", function() {
        	$("#userForm")[0].reset();
        });
